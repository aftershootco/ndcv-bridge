//! <https://docs.rs/opencv/latest/opencv/imgproc/fn.erode.html>
//! <https://docs.opencv.org/4.13.0/d4/d86/group__imgproc__filter.html#gaeb1e0c1033e3f6b891a25d0511362aeb>
use crate::conversions::*;
use nalgebra::{Point2, Vector4};
use ndarray::*;

#[derive(Debug, thiserror::Error)]
pub enum ErodeError {
    #[error("Conversion error: {0}")]
    ConversionError(#[from] crate::conversions::ConversionError),
    #[error("OpenCV error: {0}")]
    OpenCvError(#[from] opencv::Error),
}

mod seal {
    pub trait Sealed {}
    // src: input image; the number of channels can be arbitrary, but the depth should be
    // CV_8U, CV_16U, CV_16S, CV_32F or CV_64F.
    impl Sealed for u8 {}
    impl Sealed for u16 {}
    impl Sealed for i16 {}
    impl Sealed for f32 {}
    impl Sealed for f64 {}
}

pub trait NdCvErode<T: bytemuck::Pod + seal::Sealed, D: ndarray::Dimension>:
    crate::image::NdImage + crate::conversions::NdAsImage<T, D>
{
    /// Erodes an image using a structuring element with all parameters exposed.
    ///
    /// - `kernel`: 2-D structuring element
    /// - `anchor`: position of the anchor within the element; `Point2::new(-1, -1)` means the center
    /// - `iterations`: number of times erosion is applied
    /// - `border_type`: pixel extrapolation method (`BORDER_WRAP` is not supported by OpenCV erode)
    /// - `border_value`: per-channel border constant used when `border_type` is `BorderConstant`
    fn erode(
        &self,
        kernel: ndarray::ArrayView2<u8>,
        anchor: Point2<i32>,
        iterations: u16,
        border_type: crate::gaussian::BorderType,
        border_value: Vector4<f64>,
    ) -> Result<ndarray::Array<T, D>, ErodeError>;

    /// Erodes an image with default parameters: anchor at center, `BORDER_CONSTANT` border,
    /// and the morphology default border value (`f64::MIN` for all channels).
    fn erode_def(
        &self,
        kernel: ndarray::ArrayView2<u8>,
        iterations: u16,
    ) -> Result<ndarray::Array<T, D>, ErodeError> {
        let bv = opencv::imgproc::morphology_default_border_value()?.0;
        let border_value = bytemuck::cast::<[f64; 4], Vector4<f64>>(bv);
        self.erode(
            kernel,
            Point2::new(-1, -1),
            iterations,
            crate::gaussian::BorderType::BorderConstant,
            border_value,
        )
    }
}

impl<
    T: bytemuck::Pod + num::Zero + seal::Sealed,
    S: ndarray::RawData + ndarray::Data<Elem = T>,
    D: ndarray::Dimension,
> NdCvErode<T, D> for ArrayBase<S, D>
where
    ndarray::ArrayBase<S, D>: crate::image::NdImage + crate::conversions::NdAsImage<T, D>,
    ndarray::Array<T, D>: crate::conversions::NdAsImageMut<T, D>,
{
    fn erode(
        &self,
        kernel: ndarray::ArrayView2<u8>,
        anchor: Point2<i32>,
        iterations: u16,
        border_type: crate::gaussian::BorderType,
        border_value: Vector4<f64>,
    ) -> Result<ndarray::Array<T, D>, ErodeError> {
        let mut dst = ndarray::Array::zeros(self.dim());
        let cv_self = self.as_image_mat()?;
        let mut cv_dst = dst.as_image_mat_mut()?;
        let cv_kernel = kernel.as_image_mat()?;
        opencv::imgproc::erode(
            &*cv_self,
            &mut *cv_dst,
            &*cv_kernel,
            opencv::core::Point::new(anchor.x, anchor.y),
            iterations as i32,
            border_type as i32,
            opencv::core::VecN([
                border_value.x,
                border_value.y,
                border_value.z,
                border_value.w,
            ]),
        )?;
        Ok(dst)
    }
}

/// In-place variant of erosion.
pub trait NdCvErodeInPlace<T: bytemuck::Pod + seal::Sealed, D: ndarray::Dimension>:
    crate::image::NdImage + crate::conversions::NdAsImageMut<T, D>
{
    fn erode_inplace(
        &mut self,
        kernel: ndarray::ArrayView2<u8>,
        anchor: Point2<i32>,
        iterations: u16,
        border_type: crate::gaussian::BorderType,
        border_value: Vector4<f64>,
    ) -> Result<&mut Self, ErodeError>;

    fn erode_def_inplace(
        &mut self,
        kernel: ndarray::ArrayView2<u8>,
        iterations: u16,
    ) -> Result<&mut Self, ErodeError> {
        let bv = opencv::imgproc::morphology_default_border_value()?.0;
        let border_value = bytemuck::cast::<[f64; 4], Vector4<f64>>(bv);
        self.erode_inplace(
            kernel,
            Point2::new(-1, -1),
            iterations,
            crate::gaussian::BorderType::BorderConstant,
            border_value,
        )
    }
}

impl<
    T: bytemuck::Pod + num::Zero + seal::Sealed,
    S: ndarray::RawData + ndarray::DataMut<Elem = T>,
    D: ndarray::Dimension,
> NdCvErodeInPlace<T, D> for ArrayBase<S, D>
where
    Self: crate::image::NdImage + crate::conversions::NdAsImageMut<T, D>,
{
    fn erode_inplace(
        &mut self,
        kernel: ndarray::ArrayView2<u8>,
        anchor: Point2<i32>,
        iterations: u16,
        border_type: crate::gaussian::BorderType,
        border_value: Vector4<f64>,
    ) -> Result<&mut Self, ErodeError> {
        let cv_kernel = kernel.as_image_mat()?;
        let mut cv_self = self.as_image_mat_mut()?;
        unsafe {
            crate::inplace::op_inplace(&mut cv_self, |this, out| {
                opencv::imgproc::erode(
                    this,
                    out,
                    &*cv_kernel,
                    opencv::core::Point::new(anchor.x, anchor.y),
                    iterations as i32,
                    border_type as i32,
                    opencv::core::VecN([
                        border_value.x,
                        border_value.y,
                        border_value.z,
                        border_value.w,
                    ]),
                )
            })
        }?;
        Ok(self)
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::gaussian::BorderType;
    use ndarray::{Array2, Array3};

    fn rect_kernel(size: usize) -> Array2<u8> {
        Array2::ones((size, size))
    }

    #[test]
    fn test_erode_def_basic() {
        let arr = Array3::<u8>::ones((10, 10, 3));
        let res = arr.erode_def(rect_kernel(3).view(), 1).unwrap();
        assert_eq!(res.shape(), &[10, 10, 3]);
    }

    #[test]
    fn test_erode_full_params_cv2_default_border_value() {
        let arr = Array3::<u8>::ones((10, 10, 3));
        let bv = opencv::imgproc::morphology_default_border_value()
            .unwrap()
            .0;
        let border_value = bytemuck::cast::<[f64; 4], Vector4<f64>>(bv);
        let res = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                1,
                BorderType::BorderConstant,
                border_value,
            )
            .unwrap();
        assert_eq!(res.shape(), &[10, 10, 3]);
    }

    #[test]
    fn test_erode_border_value_zero() {
        let arr = Array3::<u8>::from_elem((10, 10, 3), 255);
        let res = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                1,
                BorderType::BorderConstant,
                Vector4::zeros(),
            )
            .unwrap();
        assert_eq!(res[[0, 0, 0]], 0);
        assert_eq!(res[[9, 9, 0]], 0);
        assert_eq!(res[[5, 5, 0]], 255);
    }

    #[test]
    fn test_erode_border_value_max_u8() {
        // [255.0; 4] pads with white — corner pixels of a black image get pulled up to 255
        let arr = Array3::<u8>::zeros((10, 10, 3));
        let res = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                1,
                BorderType::BorderConstant,
                Vector4::repeat(255.0),
            )
            .unwrap();
        assert!(res.iter().all(|&v| v == 0));
    }

    #[test]
    fn test_erode_border_value_f32() {
        // Same semantics but with an f32 image
        let arr = Array3::<f32>::ones((10, 10, 3));
        let res = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                1,
                BorderType::BorderConstant,
                Vector4::zeros(),
            )
            .unwrap();
        assert_eq!(res[[0, 0, 0]], 0.0);
        assert_eq!(res[[5, 5, 0]], 1.0);
    }

    #[test]
    fn test_erode_border_value_per_channel() {
        // Each channel gets its own constant: [128.0, 64.0, 32.0, 0.0]
        let arr = Array3::<u8>::from_elem((10, 10, 3), 255);
        let res = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                1,
                BorderType::BorderConstant,
                Vector4::new(128.0, 64.0, 32.0, 0.0),
            )
            .unwrap();
        assert_eq!(res[[0, 0, 0]], 128);
        assert_eq!(res[[0, 0, 1]], 64);
        assert_eq!(res[[0, 0, 2]], 32);
    }

    #[test]
    fn test_erode_shrinks_region() {
        let mut arr = Array3::<u8>::zeros((10, 10, 1));
        arr[[5, 5, 0]] = 255;
        let res = arr.erode_def(rect_kernel(3).view(), 1).unwrap();
        // The single lit pixel should have expanded to its 3x3 neighbourhood
        assert_eq!(res[[5, 5, 0]], 0);
        assert!(res.iter().all(|&v| v == 0));
    }

    #[test]
    fn test_erode_multiple_iterations() {
        // Each additional iteration peels away another layer from the bright region's border.
        let arr = Array3::<u8>::from_elem((20, 20, 1), 255);
        let res1 = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                1,
                BorderType::BorderConstant,
                Vector4::zeros(),
            )
            .unwrap();
        let res2 = arr
            .erode(
                rect_kernel(3).view(),
                Point2::new(-1, -1),
                2,
                BorderType::BorderConstant,
                Vector4::zeros(),
            )
            .unwrap();
        let count1 = res1.iter().filter(|&&v| v > 0).count();
        let count2 = res2.iter().filter(|&&v| v > 0).count();
        assert!(count2 < count1);
    }

    #[test]
    fn test_erode_different_border_types() {
        let arr = Array3::<u8>::ones((10, 10, 3));
        let bv = opencv::imgproc::morphology_default_border_value()
            .unwrap()
            .0;
        let border_value = bytemuck::cast::<[f64; 4], Vector4<f64>>(bv);
        for border_type in [
            BorderType::BorderConstant,
            BorderType::BorderReplicate,
            BorderType::BorderReflect,
            BorderType::BorderReflect101,
        ] {
            let res = arr
                .erode(
                    rect_kernel(3).view(),
                    Point2::new(-1, -1),
                    1,
                    border_type,
                    border_value,
                )
                .unwrap();
            assert_eq!(res.shape(), &[10, 10, 3]);
        }
    }

    #[test]
    fn test_erode_f32() {
        let arr = Array3::<f32>::ones((10, 10, 3));
        let res = arr.erode_def(rect_kernel(3).view(), 1).unwrap();
        assert_eq!(res.shape(), &[10, 10, 3]);
    }

    #[test]
    fn test_erode_inplace() {
        let mut arr = Array3::<u8>::ones((10, 10, 3));
        arr.erode_def_inplace(rect_kernel(3).view(), 1).unwrap();
        assert_eq!(arr.shape(), &[10, 10, 3]);
    }

    // An anchor outside the kernel bounds causes OpenCV to return an error.
    #[test]
    fn test_erode_invalid_anchor_returns_err() {
        let arr = Array3::<u8>::ones((10, 10, 3));
        let res = arr.erode(
            rect_kernel(3).view(),
            Point2::new(10, 10),
            1,
            BorderType::BorderConstant,
            Vector4::zeros(),
        );
        assert!(res.is_err());
    }

    #[test]
    fn test_erode_inplace_invalid_anchor_returns_err() {
        let mut arr = Array3::<u8>::ones((10, 10, 3));
        let res = arr.erode_inplace(
            rect_kernel(3).view(),
            Point2::new(10, 10),
            1,
            BorderType::BorderConstant,
            Vector4::zeros(),
        );
        assert!(res.is_err());
    }
}
