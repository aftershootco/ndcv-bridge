...
mod sizes {
    ...
    fn bench_box_blur_sizes_u8(size: usize) {
        let arr = Array3::<u8>::ones((size, size, 3));
        let _out = black_box(arr.box_blur((3, 3), BorderType::BorderConstant).unwrap());
    }

    #[divan::bench(args = [512, 1024, 2048, 4096, 8096, 12000])]
    fn bench_box_blur_sizes_f32(size: usize) {
...
fn bench_box_blur_kernels(kernel_size: (i32, i32)) {
    let arr = Array3::<u8>::ones((1000, 1000, 3));
    let _out = black_box(
        arr.box_blur(kernel_size, BorderType::BorderConstant)
            .unwrap(),
    );
}

#[divan::bench]
fn bench_box_blur_border_types() -> Vec<()> {
    let border_types = [
    ...
    border_types
        .iter()
        .map(|border_type| {
            let _out = black_box(arr.box_blur((3, 3), *border_type).unwrap());
        })
        .collect()
}

#[divan::bench]
fn bench_box_blur_patterns() {
...
mod realistic {
    ...
    fn small_800_600_3x3() {
        ...
        let _blurred = black_box(arr.box_blur((3, 3), BorderType::BorderConstant).unwrap());
    }

    #[divan::bench]
    fn medium_1920x1080_5x5() {
        let arr = Array3::<u8>::ones((1920, 1080, 3));
        let _blurred = black_box(arr.box_blur((5, 5), BorderType::BorderConstant).unwrap());
    }

    #[divan::bench]
    fn large_3840x2160_9x9() {
        let arr = Array3::<u8>::ones((3840, 2160, 3));
        let _blurred = black_box(arr.box_blur((9, 9), BorderType::BorderConstant).unwrap());
    }
