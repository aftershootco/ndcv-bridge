...
mod sizes {
    ...
    fn bench_box_blur_sizes_u8(size: usize) {
        ...
        let _out = black_box(
            arr.box_blur((3, 3), BorderType::BorderConstant).unwrap(),
        );
    }
...
fn bench_box_blur_kernels(kernel_size: (i32, i32)) {
    ...
    let _out = black_box(
        arr.box_blur(kernel_size, BorderType::BorderConstant).unwrap(),
    );
}
...
fn bench_box_blur_border_types() -> Vec<()> {
    ...
    border_types
        ...
        .map(|border_type| {
            let _out = black_box(
                arr.box_blur((3, 3), *border_type).unwrap(),
            );
        })
        .collect()
}
...
mod realistic {
    ...
    fn medium_1920x1080_5x5() {
        ...
        let _blurred = black_box(
            arr.box_blur((5, 5), BorderType::BorderConstant).unwrap(),
        );
    }
    ...
    fn large_3840x2160_9x9() {
        ...
        let _blurred = black_box(
            arr.box_blur((9, 9), BorderType::BorderConstant).unwrap(),
        );
    }
