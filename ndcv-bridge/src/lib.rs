...
//! `NdCvNormalize`, `NdCvWarpAffine` and `NdCvDilate` are. The `blob` module, and part of
//! `affine`, are declared `pub mod` but never re-exported, so the following does NOT compile
//! today. The `compile_fail` marker pins that gap; once the re-exports are added this doctest
//! starts failing and should be removed.
//!
//! ```compile_fail
//! use ndcv_bridge::{NdCvBlobFromImage, EstimateAffineMethod};
//! ```
//!
